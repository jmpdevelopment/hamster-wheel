...
| LLM API cost growth | Could increase per-match spend and slow adoption. | Matching is asynchronous; default scorer is OpenAI (`cloud` mode) with threshold controls. | Complete token-efficiency controls and guided local-runtime mode in Phase 2. |
