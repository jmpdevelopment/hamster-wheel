...
| External provider integration not complete | Match quality is currently capped by heuristic scoring in production default path. | OpenAI provider and provider-settings UX/persistence are complete behind provider contracts; default scorer remains local until runtime selection wiring is complete. | Finish runtime provider application/hot-switch wiring so saved provider/model/base-URL settings drive matcher execution. |
